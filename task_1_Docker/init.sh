...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -- Конвертируем поле Depart из VARCHAR в INTEGER
    ALTER TABLE user_logs ALTER COLUMN Depart TYPE INTEGER USING Depart::integer;


    -- Создание таблицу кафедр departments
    ...
    -- Создание внешнего ключа в user_logs
    ALTER TABLE user_logs ADD CONSTRAINT fk_depart 
